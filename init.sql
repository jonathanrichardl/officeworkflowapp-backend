...
-- CREATE TABLE requirements(requirementid integer PRIMARY KEY AUTO_INCREMENT,request varchar(50),expectedoutcome varchar(50),order_id integer,status bool,FOREIGN KEY(order_id) REFERENCES orders(id));
CREATE TABLE requirements(id SERIAL PRIMARY KEY,request varchar(50),expectedoutcome varchar(50),order_id varchar(37),userid varchar(37),status bool,FOREIGN KEY(order_id) REFERENCES orders(id),FOREIGN KEY (userid) references users(id));

CREATE TABLE users(id varchar(37) PRIMARY KEY, username varchar(50),pswd varchar (256),email varchar(30));
